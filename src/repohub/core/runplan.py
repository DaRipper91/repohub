...
class Plan:
    ...
    def digest(self) -> str:
        ...
        blob = json.dumps([self.folder, [(s.id, s.argv) for s in self.steps]], sort_keys=True)
...
def _has_build_script(folder: Path) -> bool:
...
def build_plan(folder: Path | str, files: frozenset[str] | None = None,
               has_build_script: Callable[[Path], bool] = _has_build_script) -> Plan:
    folder = Path(folder)
