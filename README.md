...
`repohub` is a read-only command for scripts and quick lookups. It never clones, favorites, stars, forks or runs anything, and it never changes your accounts, notes, tags, collections or settings. (`repohub releases` refreshes RepoHub's own local cache of each favorite's latest release, as the web page does, but it can never mark anything as seen.)
...
repohub favorites [--tag TAG] [--collection NAME] [--query TEXT] [--json]
repohub releases [--json]
```
