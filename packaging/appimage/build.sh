...
repo=$(cd "$here/../.." && pwd)
PYVER=3.12  # matches PBS_VERSION in pins.sh
...
. "$here/pins.sh"
echo "== bundled Python $PBS_VERSION ($A)"
eval "want=\$PBS_SHA256_$A"
curl -fsSL "https://github.com/astral-sh/python-build-standalone/releases/download/$PBS_TAG/cpython-$PBS_VERSION+$PBS_TAG-$A-unknown-linux-gnu-install_only.tar.gz" -o "$work/python.tar.gz"
echo "$want  $work/python.tar.gz" | sha256sum -c - >/dev/null || { echo "python-build-standalone checksum mismatch" >&2; exit 1; }
...
"$PY" -m pip install --disable-pip-version-check --no-compile --only-binary=:all: --require-hashes --no-deps -q -r "$repo/packaging/requirements.lock"
"$PY" -m pip install --disable-pip-version-check --no-compile --no-deps -q "$work"/wheel/repohub-*.whl
"$PY" -m pip uninstall -y -q pip setuptools >/dev/null 2>&1 || true
find "$app/opt/python" -name '__pycache__' -type d -prune -exec rm -rf {} +
"$PY" -m compileall -q -j 0 --invalidation-mode checked-hash "$app/opt/python/lib" >/dev/null 2>&1 || true
...
eval "want=\$APPIMAGETOOL_SHA256_$A"
curl -fsSL "https://github.com/AppImage/appimagetool/releases/download/$APPIMAGETOOL_VERSION/appimagetool-$A.AppImage" -o "$tool"
echo "$want  $tool" | sha256sum -c - >/dev/null || { echo "appimagetool checksum mismatch" >&2; exit 1; }
chmod +x "$tool"
